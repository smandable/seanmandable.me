...
It's also built for the months after the plan is made. Log payments — or edit them later — and it tracks your real progress — including the interest your paydown has already saved — and a debt you've covered this cycle says so instead of asking again. If one monthly payment covers several debts — a debt-management plan, say — you group them and log the payment once; it splits to the cent, follows the servicer's schedule when there is one, and records the fee the plan keeps instead of letting it flatter the projection. And when you take a refinance you modeled, you apply it in one step rather than re-entering everything.
...
On the App Store (currently v1.8). Free to use — add debts, run Snowball and Avalanche, see your debt-free date, import or scan a statement, group the debts one payment covers, back everything up to JSON, and track your progress against the plan. A one-time **$9.99 Pro** unlock (no subscription, shared across all your devices) adds the five extra methods, the what-if tools, snowflakes, refinance modeling and one-step apply, amortization schedules, PDF and calendar export, and due-date reminders. Universal across iPhone, iPad, and Mac (iOS/iPadOS 26 or macOS 26 and later).
