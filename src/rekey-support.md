...
The App Store version is the auditor, with the fix queue behind a one-time **Unlock Fixing** purchase and the destructive cleanup tooling left out. The GitHub build is the whole app, fix queue included, plus the separate `rekey-cleanup` command-line tool, as a signed, notarized `.dmg` from [GitHub Releases](https://github.com/smandable/ReKey/releases/latest). Both are universal binaries for macOS 15 or later.
...
The fix worked; that warning is the browser's own bookkeeping. Sync behavior, each store only warning about itself, and leftover duplicate entries all play a part. The full explanation is in the repo's [Help & FAQ](https://github.com/smandable/ReKey/blob/master/docs/HELP.md).
